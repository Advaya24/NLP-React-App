...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]],
        ...
        source_padded = self.vocab.src.to_input_tensor(source,
                                                       device=self.device)  # Tensor: (src_len, b)
        target_padded = self.vocab.tgt.to_input_tensor(target,
                                                       device=self.device)  # Tensor: (tgt_len, b)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> \
        ...
        X_packed = nn.utils.rnn.pack_padded_sequence(X, lengths=source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X_packed)
        enc_hiddens, _ = nn.utils.rnn.pad_packed_sequence(
            enc_hiddens,
            batch_first=True, total_length=X.shape[0])
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(dim=2)).squeeze(
            dim=2)
    ...
    def beam_search(self, src_sent: List[str], beam_size: int = 5,
        ...
        t = 0
        while len(
                completed_hypotheses) < beam_size and t < max_decoding_time_step:
            t += 1
